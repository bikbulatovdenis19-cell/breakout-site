(() => {
  ...
  Object.assign(tr.ru,{
    navAccount:'Аккаунт',accountKicker:'Доступ к BreakOut',accountTitle:'Начните бесплатно. Продолжайте, когда будете готовы.',accountIntro:'Создайте аккаунт, активируйте 7-дневный пробный период и управляйте сроком доступа в личном кабинете.',createAccount:'Создать аккаунт',accountFact1Title:'7 дней бесплатно',accountFact1Text:'Полный доступ начинается после первого успешного входа в BreakOut.',accountFact2Title:'Срок всегда под рукой',accountFact2Text:'В кабинете видно тариф, дату окончания и подключённый Mac.',accountFact3Title:'Продление без ключей',accountFact3Text:'После покупки срок доступа обновляется в аккаунте и приложении.',accountPageKicker:'Аккаунт BreakOut',accountPageSub:'',selectedAccess:'Выбранный доступ',password:'Пароль',repeatPassword:'Повторите пароль',forgotPassword:'Забыли пароль?',accountConsent:'Я принимаю условия использования и политику конфиденциальности.',sameLogin:'Этот же аккаунт используется в BreakOut для macOS и iOS.',accountSideKicker:'Личный кабинет',accountSideTitle:'Управляйте доступом к BreakOut.',accountSideIntro:'Один аккаунт для BreakOut на Mac, iPhone и сайте — со статусом доступа и связанными устройствами.',accountPreviewAccess:'Доступ',accountPreviewInactive:'Не активирован',accountPreviewPlan:'Тариф',accountPreviewDevices:'Устройства',accountSideNote:'Статус доступа появляется только после входа — без лишних пустых показателей до авторизации.',accountServerPending:'Связь с аккаунтом BreakOut настроена через Supabase.',accountResetPending:'Восстановление пароля выполняется через Supabase Auth.',accountSignedInAs:'Аккаунт',accountValidUntil:'Действует до',accountLastSync:'Обновлено',accountConnectedMac:'BreakOut для macOS',accountConnectedMobile:'BreakOut для iOS',accountRefresh:'Обновить статус',accountLogout:'Выйти',accountNewPasswordTitle:'Новый пароль',accountNewPasswordIntro:'Введите новый пароль для аккаунта BreakOut.',accountSavePassword:'Сохранить пароль',planMonthName:'1 месяц',planSixName:'6 месяцев',planYearName:'1 год',pricingIntro:'Первый успешный вход активирует 7-дневный пробный период. Дальнейший срок доступа привязывается к аккаунту BreakOut.'
  });
  Object.assign(tr.en,{
  ...
  if(signInForm && registerForm){
    ...
    const macDevicesBox=document.querySelector('[data-account-mac-devices]');
    const mobileDevicesBox=document.querySelector('[data-account-mobile-devices]');
    ...
    async function loadDashboard(){
      ...
      try{
        const [access,devices,companions]=await Promise.all([api.currentAccess(),api.devices(),api.companionDevices()]);
        ...
        if(dashboardCount)dashboardCount.textContent=`${devices?.length||0} Mac · ${companions?.length||0} iOS`;
        if(previewState)previewState.textContent=stateText(access);
        if(previewPlan)previewPlan.textContent=planText(access?.plan);
        if(previewDevices)previewDevices.textContent=`${devices?.length||0} Mac · ${companions?.length||0} iOS`;
        const renderDevices=(box,rows,kind)=>{if(!box)return;box.innerHTML='';if(!(rows||[]).length){const empty=document.createElement('div');empty.className='account-device-empty';empty.textContent=kind==='mac'?message('Mac пока не подключён.','No Mac connected yet.'):message('iPhone или iPad пока не подключён.','No iPhone or iPad connected yet.');box.appendChild(empty);return}(rows||[]).forEach(d=>{const row=document.createElement('div');row.className='account-device-row';const meta=document.createElement('div');const strong=document.createElement('strong');strong.textContent=d.name||(kind==='mac'?'Mac':'iPhone');const small=document.createElement('small');small.textContent=`${d.platform||(kind==='mac'?'macOS':'iOS')} · ${message('последняя проверка','last seen')} ${fmtDate(d.last_seen_at||d.activated_at)}`;meta.append(strong,small);const btn=document.createElement('button');btn.type='button';btn.className='account-device-revoke';btn.textContent=message('Отключить','Revoke');btn.addEventListener('click',async()=>{btn.disabled=true;try{if(kind==='mac')await api.revokeDevice(d.id);else await api.revokeCompanionDevice(d.id);await loadDashboard()}catch(e){dashboardStatus.textContent=humanError(e);dashboardStatus.className='account-status error';btn.disabled=false}});row.append(meta,btn);box.appendChild(row)})};
        renderDevices(macDevicesBox,devices,'mac');
        renderDevices(mobileDevicesBox,companions,'mobile');
